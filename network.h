...
namespace NEAT {
	...
	public:
		...
		/**** For Petri Network!!!!! **********************/
		std::vector<NNode*> places;  // NNodes that input into the network
		std::vector<NNode*> transitions; // Values output by the network
		
		bool isEnabled(NNode* t);
		bool fire(NNode* t);
		bool execute();
		...
		Network(std::vector<NNode*> ps,std::vector<NNode*> ts,std::vector<NNode*> all,int netid);

		//Same as previous constructor except the adaptibility can be set true or false with adaptval
		Network(std::vector<NNode*> ps,std::vector<NNode*> ts,std::vector<NNode*> all,int netid, bool adaptval);
