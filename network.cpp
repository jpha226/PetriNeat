...
Network::Network(std::vector<NNode*> ps,std::vector<NNode*> ts,std::vector<NNode*> all,int netid) {
  places = ps;
  transitions = ts;
  all_nodes=all;
...
Network::Network(std::vector<NNode*> ps,std::vector<NNode*> ts,std::vector<NNode*> all,int netid, bool adaptval) {
  places = ps;
  transitions = ts;
  all_nodes=all;
  name=0;   //Defaults to no name  ..NOTE: TRYING TO PRINT AN EMPTY NAME CAN CAUSE A CRASH                                    
...
bool Network::isEnabled(NNode* t) {
	...
	for(std::vector<Link*>::iterator it = t->incoming.begin(); it != t->incoming.end() && enabled; it++) {
		...
		if (((*it)->p_node)->tok_count < (*it)->weight)
			enabled = false;
...
bool Network::fire(NNode* t) {
	...
	if(isEnabled(t)) {
		...
		for(std::vector<Link*>::iterator it = t->incoming.begin(); it != t->incoming.end(); it++) {
			...
			((*it)->p_node)->tok_count = ((*it)->p_node)->tok_count - (*it)->weight;
		...
		for(std::vector<Link*>::iterator it = t->outgoing.begin(); it != t->outgoing.end(); it++) {
			...
			((*it)->p_node)->tok_count += (*it)->weight;

		}

		// Other actions taken as necessary
...
// Commenting out all neural net code
/*
